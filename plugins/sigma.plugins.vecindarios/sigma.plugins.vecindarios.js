 ...
 *
 * Admits json and gexf files based on suffix.
 *
 * This plugin also adds to the graph model a method called "vecindario".
...
(function () {
     ...
     * edges that connect two of the previously cited nodes.
     * Do that until specified deep.
    ...
    sigma.classes.graph.addMethod(
        ...
        function (centerId, deep) {
            ...
            pendingNodes.push(centerId);

            var self = this;

            /**
             * This function takes the ID of node and the deep as argument and get the nodes and edges related
             * until the deep specified. Uses recursion.
             *
             * @param  {string} nodeId  The ID of the center node.
             * @param  {number} prof Recursion deep.
             */
            function getVecinos(nodeId, prof) {

                var p = prof - 1;
                var doRecur = false;
                // Add neighbors and edges between the center and the neighbors:
                for (k1 in self.allNeighborsIndex[nodeId]) {
                    if (!localNodesIndex[k1]) {
                        localNodesIndex[k1] = true;
                        graph.nodes.push(self.nodesIndex[k1]);
                        doRecur = true;
                    }

                    for (k2 in self.allNeighborsIndex[nodeId][k1])
                        if (!localEdgesIndex[k2]) {
                            localEdgesIndex[k2] = true;
                            graph.edges.push(self.edgesIndex[k2]);
                        }
     ...
     */
    sigma.plugins.vecindarios = function () {
        var ready = false,
            readyCallbacks = [],
         ...
         * @param  {number} deep of the nodes to return.
         * @return {object}              Returns the neighborhood.
         */
        this.vecindario = function (centerNodeID, deep) {
            return graph.vecindario(centerNodeID, deep);
        };

        /**
         * This method loads the JSON graph at "object", stores it in the local graph
         * instance, and executes the callback.
         *
         * @param {Object}    graphData     The object with graph data ( nodes, edges) .
         * @param {?function} callback Eventually a callback to execute.
         */
        this.use = function (graphData, callback) {
            if (Object(graphData)) {
                graph.clear().read(graphData);
        ...
        /**
         * This method loads the GEXF graph at "path", stores it in the local graph
         * instance, and executes the callback.
         *
         * @param {string}    path     The path of the GEXF graph file.
         * @param {?function} callback Eventually a callback to execute.
         */
        this.loadGEXF = function (path, callback) {

            sigma.parsers.gexf( path, function(s){
                    db.use(s, callback);
            });
